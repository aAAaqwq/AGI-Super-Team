...
from harness_package_fixture import PACKAGES  # noqa: E402
...
#
# The four per-harness packages carry the harness-specific layer only; they do
# not vendor the canonical Skill bodies, which the installer produces from the
# single `skills/` root (ADR-0002). `tests/harness_package_fixture.py` is the
# authoritative registry the per-harness tests read; this mapping is asserted
# against it below so the two cannot drift.
DISTRIBUTION_MANIFESTS = {
    "agi-super-team-codex": ".codex-plugin/plugin.json",
    "agi-super-team-claudecode": ".claude-plugin/plugin.json",
    "agi-super-team-hermes": "plugin.json",
    "agi-super-team-dsh": "dsh-plugin.json",
    "agi-super-team-openclaw": "openclaw.plugin.json",
}
...
class DistributionReleaseContractTests(unittest.TestCase):
    ...
    def test_distribution_manifests_agree_with_the_package_registry(self) -> None:
        """The registry and the README must agree with the release contract.

        ``DISTRIBUTION_MANIFESTS`` drives the release assertions and
        ``plugins/README.md`` is what a human reads. A package missing from
        either is a package that silently stops being tested or stops being
        discoverable.

        The check is one-directional because the fixture registry covers only
        the packages whose bytes come from a harness adapter. The Codex package
        is generated by ``scripts/build_codex_csuite_adapter.py``, which renders
        its own TOML rather than delegating to ``bin/adapters/codex.mjs``, so it
        is not in that registry and is asserted here by manifest only.
        """

        registered = {spec.package: spec.manifest for spec in PACKAGES}
        for package, manifest in registered.items():
            with self.subTest(package=package):
                self.assertIn(package, DISTRIBUTION_MANIFESTS)
                self.assertEqual(DISTRIBUTION_MANIFESTS[package], manifest)

        readme = (ROOT / "plugins" / "README.md").read_text(encoding="utf-8")
        for package in DISTRIBUTION_MANIFESTS:
            with self.subTest(package=package, in_readme=True):
                self.assertIn(package, readme)
                self.assertTrue((ROOT / "plugins" / package).is_dir())
